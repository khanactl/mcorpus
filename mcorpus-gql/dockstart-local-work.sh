...
JAVA_OPTS=(
  ...
  '-cp log4j-api.jar:log4j-core.jar:log4j-slf4j-impl.jar:disruptor.jar'
)

docker run \
  --add-host=localbox:128.48.44.217 \
  -p 5150:5150/tcp -p 5432/tcp \
  -e JAVA_OPTS="${JAVA_OPTS[*]}" \
  -e MCORPUS_DB_DATA_SOURCE_CLASS_NAME='org.postgresql.ds.PGSimpleDataSource' \
  -e MCORPUS_DB_URL='jdbc:postgresql://localbox:5432/mcorpus?user=mcweb&password=mcweb&ssl=false' \
  ...
  -e RATPACK_SERVER__PUBLIC_ADDRESS='http://localhost:5150' \
  "$@"
